...
class UrlQueryParser
{
     ...
     * @throws Exception
     */
    public function createQuery($params, $limit=null, $offset=null, $sort=null, $defaultConditions, $noLimit=false )
    {
        $query = new Query;

        $fields = $this->isEnabled(self::FIELDS) ? $this->extractCommaSeparatedValues($params, 'fields') : null;
        if(!$noLimit){
            $offset = ($this->isEnabled(self::OFFSET) ? $this->extractInt($params, 'offset') : null) ?? $offset;
            $limit = ($this->isEnabled(self::LIMIT) ? $this->extractInt($params, 'limit') : null) ?? $limit;
        }
        $sort = ($this->isEnabled(self::SORT) ? $this->extractArray($params, 'sort') : null) ?? $sort;
